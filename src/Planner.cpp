...
int Planner::getRefStates(MatrixN q, MatrixN v, MatrixN vref, double z_average)
{
  ...
  */
    ...
    if (vref(5, 0) != 0)
    {
        for (int i = 0; i < n_steps; i++)
        {
            xref(0, 1 + i) = (vref(0, 0) * std::sin(vref(5, 0) * dt_vector(0, i)) + vref(1, 0) * (std::cos(vref(5, 0) * dt_vector(0, i)) - 1.0)) / vref(5, 0);
            xref(1, 1 + i) = (vref(1, 0) * std::sin(vref(5, 0) * dt_vector(0, i)) - vref(0, 0) * (std::cos(vref(5, 0) * dt_vector(0, i)) - 1.0)) / vref(5, 0);
        }
    }
    else
    {
